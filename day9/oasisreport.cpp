...
int calcRecForesee(vector<long> analisys)
{
    if (analisys.size() == 0)
        return -1;
    ...
    for (long j = 0; j < (analisys.size() - 1); j++)
    {
        allzero &= (analisys[j] == 0);
        difference.push_back(analisys[j + 1] - analisys[j]);
    }

    if (allzero)
        return 0;

    return analisys[(analisys.size() - 1)] + calcRecForesee(difference);
}
...
void calulareForesee()
{
    ...
    {
        long foreseevalue = calcRecForesee(report.foreSees[0]);
